...
// 自定义 Hook 用于获取 NFT 数据
function useNFTData(address: string | undefined) {
  const { data: userNFTCount } = useReadContract({
  ...
  useEffect(() => {
    const fetchNFTData = async () => {
      if (userNFTCount && address) {
        const count = Number(userNFTCount);
        const promises = Array.from({ length: count }).map(async (_, i) => {
          ...
          const ownerAddress = await useReadContract({
            address: houseAssetAddress,
            abi: HouseAssetABI,
...
export default function PropertyManagement() {
  ...
  }, [isMinted, isStaked, isWithdrawn, isApproved]);
